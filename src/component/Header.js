...
class Header extends Component {
  ...
  onTextChange = e => {
    ...
    if (e.key === "Enter") {
      window.location =
        window.location.origin + "/product-list?q=" + searchString;
    ...
    if (searchString?.length >= 3) {
      ProductService.fetchAllProducts({ q: searchString }).then(result => {
        this.setState({
          seachResults: result.data.length > 0 ? result.data : ["1"],
        });
      });
    } else {
  ...
  addToCart = cartProduct => {
    if (
      this.props.cart.find(
        ({ product, variation_index }) =>
          product === cartProduct.id && variation_index === 0,
      ) !== undefined
    ) {
          ...
          item?.content ? (
            <div className="result-product-wrapper" key={index}>
              <Link
                ...
                onClick={() =>
                  this.setState({ searchQuery: "", seachResults: [] })
                }
              >
              ...
              <span>
                ...
                <span className="footer-head">
                  <span className="result-cat">
                    <small>
                      {item.category?.parent_category[0].title},{" "}
                      {item.category?.cate_title}
                    </small>
                  </span>
                  {item.variation_available ? (
                    <Link
                      to={{
                        pathname: `/product-detail`,
                        search: "?pid=" + item.content.product_id,
                    ...
                    <span
                      className="result-addtocart"
                      onClick={() =>
                        this.props.cart.find(
                          ({ product, variation_index }) =>
                            product === item.id && variation_index === 0,
                        ) !== undefined
                          ? ""
                          : this.addToCart([item])
                      }
      ...
      <>
        ...
        <div>
          <Modal
            ...
            style={
              overlayType === "login" ? customLoginStyles : customCartStyles
            }
          ...
          >
            {overlayType === "login" ? (
              <Login
                dismissModal={() => this.dismissModal(overlayType)}
                {...this.state}
              />
            ) : (
              <CartOverlay
        ...
        {scrolled ? (
          <div className="headersticky fixed-header">
            <div className="appLogo">
              <Link to="/">
                <img
                  ...
                  src={require("../public/logo-eshilp.svg")}
                  alt="logoeship"
                />
              </Link>
            </div>
            ...
            <div className="appaccout">
              {" "}
              <ul>
                {this.props.userData.token ? (
                  <li
                    className="nav-item"
                    onMouseEnter={() => this.setIsMenuShown(true)}
                    onMouseLeave={() => this.setIsMenuShown(false)}
                  >
                    {" "}
                    <Link to="/my-account/dashboard" className="nav-link">
                      My Account
                    </Link>
                    {isMenuShown && (
                  ...
                  <li
                    className="nav-item"
                    onClick={() => this.dismissModal("login")}
                  >
                ...
                <li className="nav-item">
                  <Link to="/compare" className="nav-link">
                    ...
                    {this.props?.compare?.length > 0 && (
                      <span>{this.props?.compare?.length}</span>
                    )}
                  </Link>
                </li>
                ...
                <li
                  className="nav-item"
                  onClick={() => this.dismissModal("cart")}
                >
          ...
          <>
                  ...
                  <div className="col-md-6 col-6">
                    <div className="acess-container float-right">
                      <div className="access_icons">
                        <div
                          className="acess-icon balloon"
                          title="large font size"
                        >
                          <img
                            src={require("../public/increase-font-size.svg")}
                            alt=""
                          />
                        </div>
                        <div
                        ...
                        >
                          <img
                            src={require("../public/decrease-font-size.svg")}
                            alt="decrease font"
                          />
                        </div>
                        ...
                        <a className="skipcontent" href="#maincontent">
                          Skip to Content
                        </a>
                      </div>
                    </div>
            ...
            <div className="header-middle d-flex justify-content-between align-items-center px-3">
              ...
              <div
                className="search-container mx-5 w-100 position-relative"
